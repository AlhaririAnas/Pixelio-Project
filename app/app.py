...
def get_similar_images(img_path):
    ...
    if app.config["args"].color:
        ...
        _, indices = index.search(
            np.array(app.config["similarities"][id][0].reshape(1, -1)), app.config["args"].image_count + 1
        )
        print(indices)
        ...
        _, indices = index.search(
            np.array(app.config["similarities"][id][1].reshape(1, -1)), app.config["args"].image_count + 1
        )
        ...
        for image_id, vectors in app.config["similarities"].items():
            if any(item in list(vectors[2].keys()) for item in yolo_classes):
                yolo_distance = np.mean(
                    [mean_iou(similarity, vectors[2]) for similarity in [app.config["similarities"][id][2]]]
                )
                yolo_distances[image_id] = yolo_distance
        indices = [sorted(yolo_distances, key=yolo_distances.get, reverse=True)[: app.config["args"].image_count + 1]]
